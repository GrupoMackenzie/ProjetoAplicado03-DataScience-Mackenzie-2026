...
def load_artifacts():
    ...
    return games, knn, cluster_names, cluster_ranges
...
games, knn, cluster_names, cluster_ranges = load_artifacts()
...
with tab1:
    ...
    if st.session_state.get("results"):
        ...
        cols = st.columns(2)
        st.metric("CPU Necessária", r["cpu_str"] or "N/A")
        st.metric("Pontuação do CPU", f"{r['score']:.1f}" if r["score"] else "N/A")
        ...
        if r["score"] is not None:
            ...
            neighbors = games.iloc[indices[0]].copy()
                ...
                with st.form("feedback_form"):
                    ...
                    if st.form_submit_button("Enviar"):
                        ok = save_feedback(
                            ...
                            [games.loc[games.iloc[idx].name]['name']
...
with tab2:
    score_input = st.number_input("Pontuação do CPU", min_value=0.0, max_value=125.0, step=1.0, value=70.0)
    if st.button("Encontrar Similares", key="score_btn"):
        ...
        st.dataframe(
            games.loc[games.iloc[indices[0]].index][['name', 'min_cpu', 'cpu_bench']],
            width='stretch',
            hide_index=True
        )
